...
const ScrollStack = ({
  ...
  // Read card offset positions relative to the scroll container once and cache them to prevent layout thrashing
  const measureOffsets = useCallback(() => {
    if (!cardsRef.current.length) return;

    const { scrollTop } = getScrollData();

    // Cache card offsets (read phase)
    ...
    if (endElement) {
      if (useWindowScroll) {
        endOffset = endElement.getBoundingClientRect().top + scrollTop;
      } else {
  ...
  const handleScroll = useCallback(() => {
    // Request animation frame to throttle style updates and align with browser screen refresh rate
  ...
  useLayoutEffect(() => {
    ...
    // Force layout updates on resize
    const handleResize = () => {
      measureOffsets();
      updateCardTransforms();
    };
