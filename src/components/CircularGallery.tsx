...
class App {
  ...
  onTouchUp(e) {
    ...
    const endX = e.changedTouches ? e.changedTouches[0].clientX : (e.clientX !== undefined ? e.clientX : this.clickStartX);
    const endY = e.changedTouches ? e.changedTouches[0].clientY : (e.clientY !== undefined ? e.clientY : this.clickStartY);
    
    const moveX = Math.abs(endX - this.clickStartX);
    const moveY = Math.abs(endY - this.clickStartY);

    if (elapsed < 300 && moveX < 8 && moveY < 8) {
      this.handleCanvasClick(endX, endY);
    }
  }
  handleCanvasClick(screenX, screenY) {
    if (!this.container || !this.medias || !this.medias[0]) return;
    ...
    const viewportX = ndcX * (this.viewport.width / 2);
    const viewportY = ndcY * (this.viewport.height / 2);
    
    let clickedCardIndex = -1;
    for (let i = 0; i < this.medias.length; i++) {
      const media = this.medias[i];
      if (!media || !media.plane) continue;
    ...
    if (clickedCardIndex !== -1) {
      const originalIndex = clickedCardIndex % (this.medias.length / 2);
      const clickedItem = this.mediasImages[clickedCardIndex];
