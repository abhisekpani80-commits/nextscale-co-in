  ...
  return (
    <article className="min-h-screen bg-slate-50/50 text-slate-900">
      ...
      <PageHero
        ...
        title={<>AI Automation &amp; Web Development for businesses in <span className="text-blue-600">{data.name}</span>.</>}
        description={data.ecosystem}
      >
      ...
      <main className="mx-auto max-w-5xl px-6 py-16 sm:py-24 flex flex-col gap-20">
          ...
          <div>
            ...
            <div className="flex flex-col gap-6 text-slate-600 text-sm leading-relaxed">
              <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-xs">
                <h3 className="font-bold text-slate-900 mb-1.5">Dominant Industries</h3>
                <p>{data.industries}</p>
              </div>
              <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-xs">
                <h3 className="font-bold text-slate-900 mb-1.5">Local AI Adoption Trends</h3>
                <p>{data.trends}</p>
              </div>
              <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-xs">
                <h3 className="font-bold text-slate-900 mb-1.5">Local Business Challenges</h3>
                <p>{data.challenges}</p>
              </div>
            </div>
          </div>
          ...
          <aside className="bg-white border border-slate-200 p-6 rounded-2xl shadow-xs h-fit">
            ...
            <div className="flex flex-col gap-4 text-xs text-slate-600">
              <div className="flex justify-between border-b border-slate-100 pb-2">
                ...
                <span className="font-semibold text-slate-900">Remote-First</span>
              </div>
              <div className="flex justify-between border-b border-slate-100 pb-2">
                <span>Published:</span>
              ...
              <div className="flex justify-between border-b border-slate-100 pb-2">
                ...
                <span className="font-semibold text-slate-900">{data.costs.split("—")[0]}</span>
              </div>
        ...
        {/* Project Examples */}
        <section>
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-blue-600 font-semibold mb-3">Project Examples</p>
            ...
            {data.examples.map((ex, idx) => (
              <div key={idx} className="bg-white border border-slate-200 p-6 rounded-2xl shadow-xs flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-slate-600 text-sm leading-relaxed font-medium">{ex}</p>
                </div>
              </div>
            ))}
          </div>
